...
export default function App() {
  const [step, setStep] = useState(1); // 1: Upload, 2: Review Files, 'processing': Loading, 3: Dashboard
  const [isChatOpen, setIsChatOpen] = useState(false);
  const [files, setFiles] = useState([]);
  const [isDragging, setIsDragging] = useState(false);
  
  // State pentru Marketplace/Module
  const [isModulesOpen, setIsModulesOpen] = useState(false);
  const [installedModules, setInstalledModules] = useState(['crm']);
  ...
  const addFiles = (newFiles) => {
    const mockFiles = newFiles.map(file => ({
      name: file.name,
      type: file.type.includes('image') ? 'image' : file.name.includes('.pdf') ? 'pdf' : 'excel',
      size: (file.size / 1024 / 1024).toFixed(2) + ' MB'
    }));
    setFiles(prev => [...prev, ...mockFiles]);
    setStep(2); // Trecem la pasul de vizualizare
  };
  ...
  const removeFile = (indexToRemove) => {
    setFiles(prev => {
      ...
      if (newFiles.length === 0) setStep(1); // Daca nu mai sunt fisiere, inapoi la upload
      return newFiles;
    });
  };
  ...
  const processData = () => {
    ...
    setStep('processing');
    setTimeout(() => {
      setStep(3); // Dashboard-ul este acum pasul 3
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 relative overflow-hidden">
      ...
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center sticky top-0 z-10">
        ...
        {/* Meniu Module / App Store in Header */}
        <div className="hidden md:flex flex-1 justify-center">
          <button 
            onClick={() => isDataProcessed ? setIsModulesOpen(true) : null}
        ...
        <div className="flex items-center gap-2 md:gap-4 text-xs md:text-sm font-medium text-slate-500">
          <div className={`flex items-center gap-1 ${step === 1 ? 'text-indigo-600' : 'text-green-500'}`}>
            <span className="w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center border-2 border-current">1</span>
            <span className="hidden md:inline">Încărcare</span>
          </div>
          <ChevronRight className="w-4 h-4" />
          <div className={`flex items-center gap-1 ${step === 2 ? 'text-indigo-600' : (step === 3 || step === 'processing' ? 'text-green-500' : 'text-slate-400')}`}>
            <span className="w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center border-2 border-current">2</span>
            <span className="hidden md:inline">Verificare</span>
          </div>
          <ChevronRight className="w-4 h-4" />
          <div className={`flex items-center gap-1 ${step === 3 ? 'text-indigo-600' : step === 'processing' ? 'text-blue-500 animate-pulse' : 'text-slate-400'}`}>
            <span className={`w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center border-2 border-current`}>3</span>
      ...
      <main className="max-w-6xl mx-auto p-6 md:p-12">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
            <div className="text-center mb-10">
              <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4 text-slate-900">
                Transformă datele nestructurate în <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600">Inteligență Vizuală</span>
              </h1>
            ...
            <div 
              className={`border-3 border-dashed rounded-3xl p-16 text-center transition-all duration-200 bg-white ${isDragging ? 'border-indigo-500 bg-indigo-50 shadow-inner' : 'border-slate-300 hover:border-indigo-400 hover:bg-slate-50 shadow-sm'}`}
        ...
        {/* STEP 2: REVIEW FILES (Full Width) */}
        {step === 2 && (
          <div className="animate-in fade-in slide-in-from-right-8 duration-500 max-w-3xl mx-auto">
            <div className="bg-white p-8 md:p-10 rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50">
              ...
              <div className="space-y-4 mb-10 max-h-[400px] overflow-y-auto pr-2">
                {files.map((file, idx) => (
                  <div key={idx} className="flex items-center p-4 bg-slate-50 hover:bg-slate-100 transition-colors border border-slate-100 rounded-2xl gap-4 group">
        ...
        {step === 'processing' && (
          <div className="min-h-[60vh] flex flex-col items-center justify-center animate-in fade-in duration-300">
            <div className="relative w-32 h-32 mb-8">
              ...
              <div className="absolute inset-2 bg-purple-500 rounded-full animate-pulse opacity-40"></div>
            ...
            <div className="w-64 h-2 bg-slate-100 rounded-full mt-8 overflow-hidden">
              <div className="h-full bg-gradient-to-r from-indigo-500 to-purple-500 w-full rounded-full origin-left animate-[progress_3s_ease-in-out]"></div>
            </div>
          </div>
        )}
          ...
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="flex justify-between items-end mb-8">
              <div>
                ...
                <p className="text-slate-500">Iată informațiile extrase din cele {files.length} fișiere încărcate de tine.</p>
              </div>
              <button onClick={() => setStep(1)} className="text-sm font-medium text-indigo-600 hover:text-indigo-800">
                + Încarcă date noi
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              {[
                ...
                { label: 'Facturi Procesate', value: files.length * 15, icon: <FileText className="text-blue-500 w-6 h-6" />, change: 'Azi' },
                ...
                <div className="h-64 flex items-end gap-3 justify-between mt-6">
                  {[40, 70, 45, 90, 65, 100, 85].map((val, i) => (
                    <div key={i} className="w-full flex flex-col items-center gap-2 group">
                      {/* Valoarea fixată deasupra barei */}
                      <span className="text-xs font-bold text-slate-700 bg-slate-100 px-1.5 py-0.5 rounded-md">{val * 120}K</span>
                      <div className="w-full bg-indigo-50 rounded-t-md relative flex justify-center group-hover:bg-indigo-100 transition-colors" style={{ height: `${val}%` }}>
                        <div className={`w-full bg-gradient-to-t from-indigo-500 to-purple-500 rounded-t-md absolute bottom-0 transition-all duration-1000 ease-out`} style={{ height: `100%` }}></div>
                      </div>
                      <span className="text-xs text-slate-500 font-medium">{['Ian', 'Feb', 'Mar', 'Apr', 'Mai', 'Iun', 'Iul'][i]}</span>
                    </div>
                  ))}
                </div>
              </div>

              {/* Breakdown List (Simulating Pie/List data) */}
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <h3 className="font-bold text-lg mb-6 flex items-center gap-2">Categorii de Cheltuieli</h3>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-md h-full shadow-2xl animate-in slide-in-from-right-full duration-300 flex flex-col">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              <button onClick={() => setIsModulesOpen(false)} className="p-2 bg-white hover:bg-slate-100 rounded-full shadow-sm border border-slate-200 transition-colors text-slate-500">
                <X className="w-5 h-5"/>
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-slate-50/50">
              {[
                { id: 'crm', name: 'CRM Integrat', icon: <Users className="w-6 h-6 text-blue-500"/>, desc: 'Gestionează clienții, prospecții și pipeline-ul de vânzări direct din platformă.' },
                { id: 'ecommerce', name: 'eCommerce', icon: <ShoppingCart className="w-6 h-6 text-emerald-500"/>, desc: 'Lansează rapid un magazin online B2B/B2C sincronizat cu produsele și stocurile.' },
                { id: 'erp', name: 'ERP Cloud', icon: <Briefcase className="w-6 h-6 text-amber-500"/>, desc: 'Sistem avansat pentru planificarea resurselor, facturare, contabilitate.' },
                { id: 'landing', name: 'Landing Page AI', icon: <Layout className="w-6 h-6 text-purple-500"/>, desc: 'Generare automată de website-uri de prezentare bazate pe portofoliul și datele firmei.' }
              ].map(mod => {
                const isInstalled = installedModules.includes(mod.id);
                return (
                  <div key={mod.id} className="bg-white border border-slate-200 rounded-2xl p-4 flex gap-4 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all group">
                    ...
                    <div className="flex-1">
                      ...
                      <button 
                        onClick={() => {
                          if(!isInstalled) setInstalledModules(prev => [...prev, mod.id]);
                        }}
                        className={`text-xs font-bold px-4 py-2 rounded-lg flex items-center justify-center gap-2 w-full transition-colors border ${isInstalled ? 'bg-green-50 border-green-200 text-green-700 cursor-default' : 'bg-white border-indigo-200 text-indigo-700 hover:bg-indigo-600 hover:text-white'}`}
                      >
                        {isInstalled ? <><CheckCircle2 className="w-4 h-4"/> Instalat</> : <><Download className="w-4 h-4"/> Instalează</>}
                      </button>
                    </div>
                  </div>
                )
      ...
      {/* FLOATING ASK AI BUTTON - UPDATED FOR LOCKED STATE */}
      <button 
        onClick={() => isDataProcessed ? setIsChatOpen(true) : null}
      ...
      >
        <div className={`p-2 rounded-full transition-colors ${isDataProcessed ? 'bg-gradient-to-tr from-indigo-500 to-purple-500' : 'bg-slate-100'}`}>
          {isDataProcessed ? <Sparkles className="w-5 h-5 text-white" /> : <Lock className="w-5 h-5 text-slate-400" />}
        </div>
        <span className="font-bold text-sm">{isDataProcessed ? 'Întreabă AI-ul' : 'AI Indisponibil'}</span>
      </button>
            ...
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === 'user' ? 'bg-slate-800' : 'bg-gradient-to-tr from-indigo-600 to-purple-600'}`}>
                    {msg.role === 'user' ? <span className="text-white font-bold text-sm">EU</span> : <Sparkles className="w-5 h-5 text-white" />}
                  </div>
                  <div className={`p-4 rounded-2xl text-sm md:text-base leading-relaxed ${msg.role === 'user' ? 'bg-slate-100 text-slate-800 rounded-tr-none' : 'bg-white border border-slate-200 shadow-sm rounded-tl-none text-slate-700'}`}>
                    {msg.text}
                  </div>
                </div>
              </div>
