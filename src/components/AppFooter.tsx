...
const productLinks = ['Funcționalități', 'Prețuri', 'Integrări', 'Actualizări', 'API'];
const companyLinks = ['Despre noi', 'Cariere', 'Blog', 'Contact', 'Parteneri'];
        ...
        <footer className="bg-white border-t border-slate-200 mt-auto">
            <div className="max-w-6xl mx-auto px-6 py-12">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                    ...
                    <FooterLinks title="Produs" links={productLinks} />
                    <FooterLinks title="Companie" links={companyLinks} />
                </div>
    ...
    return (
        <div>
            ...
            <ul className="space-y-2 text-sm text-slate-500">
                {links.map((link) => <li key={link}><a href="#" className="hover:text-indigo-600 transition-colors">{link}</a></li>)}
            </ul>
        </div>
    );
}
