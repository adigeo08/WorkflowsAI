...
export function ChatWidget({ isOpen, messages, input, chatEndRef, isDataProcessed, onToggle, onClose, onInputChange, onSubmit }: Props) {
    return (
        <>
            <button
                type="button"
                ...
                className={`fixed bottom-5 right-5 z-30 rounded-full border-2 p-4 shadow-2xl transition-all md:bottom-8 md:right-8 ${
                    isDataProcessed
            ...
            {isOpen && isDataProcessed && (
                <div className="fixed inset-x-3 bottom-24 top-20 z-30 flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl animate-in slide-in-from-bottom-5 fade-in duration-300 sm:left-auto sm:right-8 sm:top-auto sm:h-[500px] sm:w-96">
                    ...
                    <form onSubmit={onSubmit} className="p-3 border-t border-slate-100 flex gap-2 shrink-0 sm:p-4">
                        <input type="text" value={input} onChange={(e) => onInputChange(e.target.value)} placeholder="Întreabă-mă orice..." className="min-w-0 flex-1 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 transition-all" />
                        <button type="submit" className="shrink-0 bg-indigo-600 text-white p-2 rounded-xl hover:bg-indigo-700 transition-colors"><Send className="w-4 h-4" /></button>
                    </form>
                </div>
            )}
        </>
