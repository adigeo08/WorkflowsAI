                            ...
                            <div key={mod.id} className="bg-white border border-slate-200 rounded-2xl p-4 flex gap-4 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all group">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex gap-2">
                                        <button type="button" onClick={() => onToggleModule(mod.id)} className={`text-xs font-bold px-4 py-2 rounded-lg flex items-center justify-center gap-2 flex-1 transition-colors border ${isInstalled ? 'bg-red-50 border-red-200 text-red-700 hover:bg-red-100' : 'bg-white border-indigo-200 text-indigo-700 hover:bg-indigo-600 hover:text-white'}`}>
                                            {isInstalled ? <><Trash2 className="w-4 h-4" />Dezinstalează</> : <><Download className="w-4 h-4" />Instalează</>}
                                        </button>
                                        {isInstalled && (
                                            <button type="button" onClick={() => onPreviewModule(mod.id)} className="text-xs font-bold px-4 py-2 rounded-lg flex items-center justify-center gap-2 bg-indigo-50 border border-indigo-200 text-indigo-700 hover:bg-indigo-100 transition-colors">
