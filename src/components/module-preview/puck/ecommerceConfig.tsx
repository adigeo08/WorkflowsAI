...
export const createEcommercePuckConfig = (catalogItems: CatalogItem[] = initialCatalogItems): Config => ({
  components: {
    StoreHeader: {
      ...
      render: ({ logo, menuItems, cartLabel }: StoreHeaderProps) => (
    ...
    StoreHero: {
      ...
      render: ({ badge, title, subtitle, primaryCta, accentColor, backgroundColor, textColor }: StoreHeroProps) => (
    ...
    PromoBanner: { fields: { title: textField, subtitle: { type: 'textarea' }, cta: textField, backgroundColor: textField }, defaultProps: { title: 'Upgrade pentru echipe', subtitle: 'Pachete speciale pentru birouri moderne și showroom-uri gadget.', cta: 'Vezi oferta', backgroundColor: '#4f46e5' }, render: ({ title, subtitle, cta, backgroundColor }: PromoBannerProps) => <section className="m-6 rounded-[2rem] p-8 text-white" style={{ backgroundColor }}><h2 className="text-3xl font-black">{title}</h2><p className="mt-2 opacity-80">{subtitle}</p><button className="mt-5 rounded-xl bg-white px-5 py-3 font-black text-slate-950">{cta}</button></section> },
    CheckoutBlock: { fields: { title: textField, steps: textField, paymentText: textField, shippingText: textField }, defaultProps: { title: 'Checkout rapid', steps: 'Date client, Livrare, Plată', paymentText: 'Plată securizată cu cardul', shippingText: 'Livrare în 24-48h pentru produsele în stoc' }, render: ({ title, steps, paymentText, shippingText }: CheckoutBlockProps) => <section className="px-6 pb-12"><div className="rounded-3xl border border-slate-200 bg-slate-50 p-6"><h2 className="text-2xl font-black text-slate-950">{title}</h2><p className="mt-3 font-bold text-slate-600">{steps}</p><div className="mt-4 grid gap-3 md:grid-cols-2"><p className="rounded-2xl bg-white p-4 text-sm font-bold text-slate-700">{paymentText}</p><p className="rounded-2xl bg-white p-4 text-sm font-bold text-slate-700">{shippingText}</p></div></div></section> }
  }
});

export const ecommercePuckConfig = createEcommercePuckConfig();
